...
class FieldPlayers {
    ...
    layoutTeam(team) {
        if (team && Array.isArray(team) && team.length) {
            ...
            row.style.height = `${team.length > 1 ? "25%" : "100%"}`; // ensure centered when only one
            ...
            let row_height = Math.floor(
                (100 - 25) / Math.max(1, Math.ceil(len / 4))
            );
            for (let i = 0; i < len; i++) {
                if (i > 0 && i % 4 === 0) {
                    ...
                    container.childNodes.forEach(
                        (node) =>
                            (node.style.width = `${
                                100 / container.childNodes.length
                            }%`)
                    );
                    console.log("agree");
                    let row = this.createFieldRow();
                ...
                container.appendChild(
                    this.createPlayerNode(_team.splice(0, 1)[0]?.name, i + 3)
                );
            }
    ...
    formatName(name) {
        if (name)
            return name.replace(/(\w+)\s+(\w+)/g, (match, g1, g2) =>
                g1 && g2
                    ? `${g1.charAt(0).toUpperCase()}. ${
                          g2.charAt(0).toUpperCase() + g2.slice(1)
                      }`
                    : match
            );
        return name;
