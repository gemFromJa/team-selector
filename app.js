...
const teamSizeInput = document.querySelector("#team_size");
const randomModeCheckbox = document.querySelector("#random_mode");
const teamCountInput = document.querySelector("#team_count");
let teamsNames = document.querySelector(".team_names");

function addTeamNames(count) {
	for (let i = 0; i < count; i++) {
		let id = `team_name_${teamsNames.childNodes.length + i}`;
		teamsNames.appendChild(createTeamNameInput(id));
	}
}

function createTeamNameInput(id) {
	let label = document.createElement("label");
	let span = document.createElement("span");
	let textInput = document.createElement("input");

	label.setAttribute("class", "team_input_label team1_name");

	span.setAttribute("class", "input_label_margin_btm");
	span.innerHTML = "Team Name";

	textInput.setAttribute("class", "team_input team_name_input");
	textInput.setAttribute("id", id);
	textInput.setAttribute("placeholder", " - team name - ");
...
function createSimplePlayerInput(id) {
	...
	textArea.setAttribute("placeholder", "first lastname, skill (/10), fitness (/10), size(/10)");
	return textArea;
}
...
function createTeam() {
	...
	if (check) {
		...
		console.log(`textContent: ${textArea.textContent}`, `Value:  '${textArea.value}'`);

		if (!textArea.value) return;
		// get all specific stat of a specific player
		let persons = textArea.value.trim().split("\n");
		players = persons.map((person, index) => {
			let [name, skill, fitness, size] = person.split(",").map((val) => val.trim());
	...
	// find the positive differenct
	const difOfLargest = (a, b) => Math.abs(b - a);
	...
	let teams = [Array(totalTeams).fill([])];
	let sum1 = 0,
		sum2 = 0;
	...
	if (randomModeCheckbox.checked) {
		...
		for (let i = 0; i < total_players; i++) {
			let position = Math.floor(Math.random() * _players.length);
			let player = _players.splice(position, 1)[0];
		...
		// loop through all playes
		for (let i = 0; i < total_players; i++) {
			let chosenIdx = -1;
			// if selected already, skip over
			...
			for (let j = i + 1; j < total_players; j++) {
				...
				if (chosenIdx == -1) {
					...
					difOfLargest(players[i].skill_level, players[chosenIdx].skill_level) >
			...
			if (chosenIdx === -1) {
				...
				if (
					difOfLargest(sum1 + players[i].skill_level, sum2 + chosen.skill_level) <
					difOfLargest(sum1 + chosen.skill_level, sum2 + players[i].skill_level)
				) {
					team1.push(players[i]);
					sum1 += players[i].skill_level;
					team2.push(chosen);
...
function decreaseTeamNames(count) {
	for (let i = 0; i < count; i++) {
		teamsNames.removeChild(teamsNames.lastChild);
	}
}
...
const updateTeamCount = debounce((event) => {
	console.log("@ Updating team Count @");
	let count = Number(event.target.value) || 0;

	if (count > 6 || count < 2) {
		return (event.target.value = totalTeams);
	}

	if (count < totalTeams) {
		decreaseTeamNames(totalTeams - count);
	}

	if (count > totalTeams) {
		addTeamNames(count - totalTeams);
	}
...
function Init() {
	...
	// teamCountInput.addEventListener("input", updateTeamCount);
	console.log(playerDivInput, "Chicken Burr");

	totalTeams = 2; //teamCountInput.value;
	// addTeamNames(totalTeams);
